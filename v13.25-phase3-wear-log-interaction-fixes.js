...
(function(){
  ...
  function installStyles(){
    ...
    style.id=STYLE_ID;
    style.textContent=`
      #journalDetailDialog .v1325-day-classifiers{justify-content:center!important;align-items:center!important;gap:7px!important;text-align:center!important}
      #journalDetailDialog .v1325-day-classifier-label{font-family:var(--serif)!important;font-size:.84rem!important;line-height:1!important;font-weight:700!important;color:var(--coffee)!important;margin-right:1px!important}
      ...
      #journalDetailDialog .v1325-color-popover{position:fixed!important;left:50%!important;right:auto!important;top:50%!important;transform:translate(-50%,-50%)!important;width:min(310px,calc(100vw - 28px))!important;max-width:310px!important;z-index:10050!important;margin:0!important}
      #journalDetailDialog .v1325-color-row{grid-template-columns:58px repeat(5,minmax(0,1fr))!important}
  ...
  function dedupeEraAndColor(d){
    ...
    if(keepEra){const select=keepEra.querySelector('select');if(select&&selected)select.value=selected;if(keepEra.parentNode!==strip)strip.appendChild(keepEra);}
    const colors=[...d.querySelectorAll('.v1325-day-color-wrap')];const keepColor=colors.shift()||null;colors.forEach(node=>node.remove());if(keepColor&&keepColor.parentNode!==strip)strip.appendChild(keepColor);
    const labels=[...strip.querySelectorAll('.v1325-day-classifier-label')];let label=labels.shift();labels.forEach(node=>node.remove());if(!label){label=document.createElement('span');label.className='v1325-day-classifier-label';label.textContent='Chapter';strip.prepend(label);}label.textContent='Chapter';
    strip.appendChild(label);if(keepEra)strip.appendChild(keepEra);if(keepColor)strip.appendChild(keepColor);
  }
  ...
  function lockFooter(d,editing){
    if(!d)return;d.classList.toggle('v1325-journal-editing',editing);
    const view=d.querySelector('#v1325JournalViewBtn');if(view){view.disabled=editing;view.setAttribute('aria-disabled',editing?'true':'false');}
    ['#editJournalDetailBtn','#journalOpenBoardBtn','#cancelJournalDetailBtn','#deleteJournalDetailBtn'].forEach(sel=>{const btn=d.querySelector(sel);if(btn)btn.disabled=editing;});
  }
  ...
  function returnReaderToWearLog(event){
    ...
    if(d&&!d.open){
      try{
        if(typeof lockPageForJournalDetail==='function')lockPageForJournalDetail();
        d.showModal();
        requestAnimationFrame(()=>{
          syncAll();
          window.AudreyJournalEditorLayout?.refresh?.();
          window.AudreyJournalEditorRefinements?.refresh?.();
  ...
  function bindDialogCloseRefresh(d){if(!d||d.dataset.v1325BrowseRefreshBound==='1')return;d.dataset.v1325BrowseRefreshBound='1';d.addEventListener('close',refreshBrowseRows);}

  function syncAll(){
    if(syncing)return;syncing=true;
    try{installStyles();const d=dialog();if(!d)return;dedupeEraAndColor(d);dedupeEditControls(d);syncEditState(d);bindDialogCloseRefresh(d);}finally{syncing=false;}
  }
  ...
  if(typeof openJournalDetail==='function'&&!openJournalDetail.__wearInteractionFixWrapped){
    ...
    openJournalDetail=function(){const out=open0.apply(this,arguments);requestAnimationFrame(syncAll);setTimeout(syncAll,70);setTimeout(syncAll,180);setTimeout(syncAll,340);return out;};
