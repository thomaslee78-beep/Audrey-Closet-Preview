 ...
 * Layout26 stability overlay for all three Journal surfaces.
 * Prevents horizontal drift, makes the layered Wear Log open atomic so older
 * presentation passes cannot visibly repaint after first display, and keeps
 * Journal View clothing thumbnails a consistent horizontally-scrollable size.
 */
(function(){
  'use strict';
  ...
  function settleDetail(dialog){
    ...
    /* Final APIs are idempotent; one last ordered pass ensures the newest
       hierarchy owns the DOM after legacy toolbar/layout passes have run. */
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest?.('#v1325JournalEditToggle,#v1325SaveJournalBtn,#v1325JournalViewBtn')){requestAnimationFrame(sync);setTimeout(sync,60);}
  },true);

  installStyles();wrapRenderJournal();wrapOpenDetail();wrapReader();requestAnimationFrame(sync);setTimeout(sync,150);
  window.AudreyJournalLayoutHardening={version:VERSION,refresh:sync,atomicOpen:true};
})();
