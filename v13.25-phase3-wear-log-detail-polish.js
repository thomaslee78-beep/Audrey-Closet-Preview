 ...
 * Curated day-color palette and compact detail controls.
 * v1.1 makes Day Color a dialog-wide singleton so later layout overlays can
 * safely relocate it without creating duplicate controls.
 */
(function(){
  'use strict';
  ...
  function installStyles(){
    ...
    const style=document.createElement('style');style.id=STYLE_ID;style.textContent=`
      ...
      #journalDetailDialog .v1325-detail-favorite-corner #journalDetailFavoriteBtn{width:36px!important;height:36px!important;min-width:36px!important;padding:0!important;border:1px solid rgba(108,81,66,.18)!important;border-radius:50%!important;background:rgba(255,253,248,.96)!important;color:#a15352!important;box-shadow:0 3px 10px rgba(61,48,39,.08);font-size:0!important;line-height:34px!important}
      ...
      #journalDetailDialog .v1325-day-color-btn{width:34px;height:32px;min-width:34px;display:flex;align-items:center;justify-content:center;padding:0;border:1px solid rgba(108,81,66,.18);border-radius:9px;background:#fffdf8;color:var(--coffee);font:inherit}
      #journalDetailDialog .v1325-day-color-chip{width:20px;height:20px;border-radius:5px;background:var(--day-color,#eee6d8);box-shadow:inset 0 0 0 1px rgba(65,52,44,.12)}
      ...
      @media(max-width:520px){#journalDetailDialog .v1325-journal-meta-toolbar{gap:6px!important;padding:6px!important}#journalDetailDialog .v1325-meta-rate .v1325-meta-label{display:inline!important;font-size:.64rem!important}#journalDetailDialog .v1325-color-popover{left:-72px;width:min(278px,calc(100vw - 34px))}#journalDetailDialog .v1325-color-row{grid-template-columns:50px repeat(5,1fr);gap:5px}#journalDetailDialog .v1325-color-swatch{width:28px;height:27px}}
  ...
  function buildPalette(wrap){
    let pop=wrap.querySelector('.v1325-color-popover');if(pop)return pop;
    ...
    Object.entries(PALETTE).forEach(([label,colors])=>{
      const row=document.createElement('div');row.className='v1325-color-row';row.innerHTML=`<span class="v1325-color-row-label">${label}</span>`;
  ...
  function syncColorControl(dialog,toolbar,entry){
    dialog.querySelectorAll('.v1325-meta-color').forEach(node=>node.remove());
    const wraps=[...dialog.querySelectorAll('.v1325-day-color-wrap')];
    let wrap=wraps.shift();wraps.forEach(node=>node.remove());
    if(!wrap){
      wrap=document.createElement('div');wrap.className='v1325-day-color-wrap';
      wrap.innerHTML='<button type="button" class="v1325-day-color-btn" aria-label="Choose day color" title="Day color"><span class="v1325-day-color-chip"></span></button>';
      toolbar.appendChild(wrap);
    }
    const button=wrap.querySelector('.v1325-day-color-btn');
    if(button&&!button.dataset.bound){
      button.dataset.bound='1';
      button.onclick=event=>{event.preventDefault();event.stopPropagation();const pop=buildPalette(wrap);pop.hidden=!pop.hidden;};
    }
    const color=validColor(entry?.dayColor);
    if(button){button.style.setProperty('--day-color',color||'#eee6d8');button.classList.toggle('empty',!color);button.setAttribute('aria-label','Choose day color');button.title='Day color';}
  ...
  function moveFavorite(dialog){
    ...
    let slot=head.querySelector('.v1325-detail-favorite-corner');if(!slot){slot=document.createElement('div');slot.className='v1325-detail-favorite-corner';head.appendChild(slot);}if(favorite.parentNode!==slot)slot.appendChild(favorite);
    dialog.querySelector('.v1325-meta-favorite')?.remove();
  }
  function syncRating(toolbar){const rate=toolbar.querySelector('.v1325-meta-rate');if(!rate)return;let label=rate.querySelector('.v1325-meta-label');if(!label){label=document.createElement('span');label.className='v1325-meta-label';rate.prepend(label);}label.textContent='Rating';}
  function moveJournalView(dialog){const view=dialog.querySelector('#v1325JournalViewBtn'),modeRow=dialog.querySelector('.v1325-journal-mode-row');if(!view||!modeRow)return;view.classList.add('v1325-journal-view-launch');view.textContent='Journal View';view.title='Open Journal View';if(view.parentNode!==modeRow)modeRow.prepend(view);dialog.querySelector('.v1325-meta-journal-view')?.remove();}
  ...
  function syncDetail(){
    ...
    try{
      ...
      if(!toolbar){toolbar=document.createElement('div');toolbar.className='v1325-journal-meta-toolbar';dialog.querySelector('.sheet-head')?.after(toolbar);}
      moveFavorite(dialog);syncRating(toolbar);syncColorControl(dialog,toolbar,entry);moveJournalView(dialog);
      window.AudreyWearLogLayout2?.refresh?.();
    }finally{syncing=false;}
  }

  function wrapOpenJournalDetail(){if(typeof openJournalDetail!=='function'||openJournalDetail.__wearDetailPolishWrapped)return;const open0=openJournalDetail;openJournalDetail=function(){const out=open0.apply(this,arguments);requestAnimationFrame(syncDetail);setTimeout(syncDetail,60);setTimeout(syncDetail,160);return out;};openJournalDetail.__wearDetailPolishWrapped=true;}
  document.addEventListener('click',event=>{if(!event.target.closest?.('.v1325-day-color-wrap'))document.querySelectorAll('.v1325-color-popover').forEach(p=>p.hidden=true);},true);
