...
(function(){
  ...
  function rowDisplayItems(entry){
    const sorted=orderedItems(entry),primary=[],seen=new Set();
    for(const item of sorted){const category=itemCategory(item);if(!seen.has(category)){primary.push(item);seen.add(category);}if(primary.length===2)break;}
    for(const item of sorted){if(primary.length>=2)break;if(!primary.includes(item))primary.push(item);}
    return [...primary,...sorted.filter(item=>!primary.includes(item))].slice(0,4);
  }
  ...
  function itemThumbs(entry){
    const items=rowDisplayItems(entry);
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      @media(max-width:520px){
        .journal-row.v1325-final-row .v1325-simple-log{grid-template-columns:54px 120px minmax(0,1fr)!important;gap:5px!important;min-height:102px!important;padding:8px!important}
  ...
  async function toggleFavorite(entry,button){
    if(!entry)return;const prior=favoriteValue(entry);entry.favorite=!prior;entry.updated=Date.now();
  ...
  function renderRow(row,entry){
    if(!row||!entry)return;row.classList.add('v1325-simple-log-row','v1325-final-row');row.style.setProperty('--journal-day-accent',validColor(entry.dayColor)?entry.dayColor:NEUTRAL_ACCENT);
    row.querySelector('.v1325-simple-log')?.remove();
    const date=formatCalendar(entry.date),rating=ratingValue(entry),favorite=favoriteValue(entry),title=storedTitle(entry),summary=document.createElement('div');
    ...
    summary.addEventListener('click',event=>{if(event.target.closest('.v1325-row-favorite'))return;open(event);});summary.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){open(event);}});
    const fav=summary.querySelector('.v1325-row-favorite');fav?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();toggleFavorite(entry,fav);});row.appendChild(summary);
  }

  function renderAll(){installStyles();document.querySelectorAll('.journal-row[data-journal-id]').forEach(row=>renderRow(row,entryById(row.dataset.journalId)));window.AudreyJournalLayoutHardening?.refresh?.();}
  function settleRows(){renderAll();requestAnimationFrame(renderAll);}
  function bindDetailClose(){const dialog=document.querySelector('#journalDetailDialog');if(!dialog||dialog.dataset.v1325FinalRowsCloseBound==='1')return;dialog.dataset.v1325FinalRowsCloseBound='1';dialog.addEventListener('close',()=>{requestAnimationFrame(renderAll);setTimeout(renderAll,35);});}
  function wrapRenderJournal(){if(typeof renderJournal!=='function'||renderJournal.__finalRowsWrapped)return;const render0=renderJournal;renderJournal=function(){const out=render0.apply(this,arguments);requestAnimationFrame(renderAll);setTimeout(renderAll,35);return out;};renderJournal.__finalRowsWrapped=true;}

  installStyles();wrapRenderJournal();bindDetailClose();settleRows();setTimeout(renderAll,50);
  window.AudreyJournalFinalRows={version:VERSION,refresh:renderAll,orderedItems};
})();
